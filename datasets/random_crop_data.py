...
class RandomCropData:
    ...
    def __init__(self, size=(512, 512),max_tries=10,min_crop_side_ratio=0.1):
        self.size = size
    ...
    def __call__(self, data):
        ...
        crop_x, crop_y, crop_w, crop_h = self.crop_area(img, all_care_polys)    # 切割
        ...
        for line in data['polys']:
            ...
            if not self.is_poly_outside_rect(poly, 0, 0, w, h): # 去掉切取图像以外的文本坐标
    ...
    def crop_area(self, img, polys):
        ...
        for points in polys:
            points = np.round(points, decimals=0).astype(np.int32) # 四舍五入到整数
            # 分别找到最小的x,y 和 最大的x,y
